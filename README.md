...
- **Low CPU streaming** — the sensor stream is capped at 1 fps via
  ...
  ~2.5% even while idle.
- **Unbreakable-by-design loop** — this project started as a long-running
  24/7 service and has been hardened against the problems that killed it.
  See [Reliability engineering](#reliability-engineering) below.
- **On-ramdisk logging** — every capture, window change and error is logged
  ...
  can never crowd out the image (see [Logging](#logging)).
...
- Output and log written to a `tmpfs` RAM disk (see configuration)
...
2. Edit the `CONFIG` section: set your latitude, longitude and timezone.
...
| `LOG_BACKUP_COUNT`        | 2                               | ...keeping at most 3MB of logs on the ramdisk      |
...
| `LAST_IMAGE_BANNER`       | `"New Images Start Tomorrow At Sunrise"` | Text on the last image of the day          |
...
beyond ~3MB — leaving ~29MB of the 32MB RAM disk free for the image. A
full disk can never block a capture.
...
3. When picamera2's camera setup fails partway, the object's `__del__`
   calls `close()`, which crashed with
   ...
   **before** it could reach `camera.release()` / `_cm.cleanup()`. Each
   ...
   dma-bufs).
4. The process leaked file descriptors until it hit the 1024-FD ulimit.
   From then on **every** camera init failed with `[Errno 24] Too many
   open files` — and because the retry loop never exited the process,
   systemd's `Restart=always` could never recycle it. 1100+ failures
   later it was still spinning, taking zero photos.

The diagnosis: `ls /proc/<pid>/fd | wc -l` showed exactly 1024 FDs, ~493
of them leaked pipe pairs, and the journal showed the `__del__`/`close()`
...
1. **Bounded init retries, then give up** — `init_camera()` now tries at
   ...
   (`sys.exit(1)`). systemd's `Restart=always` starts a fresh process with
   a clean file-descriptor table. A wedged camera can no longer wedge the
   process forever.
2. **Explicit teardown** — `teardown_camera()` calls both `stop()` and
   `close()` (guarded), so a fully-initialized camera releases libcamera
   resources deterministically instead of relying on the crash-prone
   `__del__` destructor.
3. **FD guard-rail** — each capture loop checks `/proc/self/fd`. Above
   `FD_WARN_THRESHOLD` it logs a warning; above `FD_EXIT_THRESHOLD` it
   tears the camera down and exits so systemd restarts cleanly. Combined
   with a raised `LimitNOFILE`, this turns any future leak into a
   self-healing restart rather than a permanent outage.
4. **systemd watchdog** — the service override sets `WatchdogSec=180` and
   the script calls `sd_notify("WATCHDOG=1")` after every successful
   capture/transition, so systemd kills and restarts the process if it
   ever hangs.
5. **Capture self-healing** — if a capture raises mid-day, the script
   tears down the camera and re-initializes on the next iteration instead
   of crashing out of the day.
6. **Closed image handles** — `stamp_photo()` now opens images with
   context managers so every PIL file handle is released, eliminating a
   secondary FD-leak vector.
...
- The script considers itself "in the window" from
  `sunrise - SUNRISE_OFFSET_MINUTES` to
  `sunset + SUNSET_OFFSET_MINUTES`.
...
- During the night it wakes every `NIGHT_POLL_SECONDS` to check the time;
  at the next morning's window start it re-initializes the camera and
  resumes.
...
- **2026-08-09** — Fixed stamp to use actual capture time instead of
  process time.
- **2026-08-11** — **Fixed `[Errno 24] Too many open files` outage.**
  Bounded camera-init retries (then process exit for systemd restart),
